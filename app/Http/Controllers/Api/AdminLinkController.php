...
class AdminLinkController extends Controller
{
    ...
    public function preview(Link $link): JsonResponse
    {
        ...
        if (!$generated) {
            try {
                $client = new Client();
                $response = $client->get('https://www.googleapis.com/pagespeedonline/v2/runPagespeed?url='.$link->url.'&screenshot=true');
                $content = json_decode($response->getBody()->getContents(), true);
                if (isset($content['screenshot']['data'])) {
                    $data = $content['screenshot']['data'];
                    $data = str_replace(['_', '-'], ['/', '+'], $data);
                    $data = base64_decode($data);
                    Storage::disk('private')->put($preview, $data);
                    return responder()->success([
                        'mime' => $content['screenshot']['mime_type'],
